...
controllerB::controllerB(MatrixXf &A, MatrixXf &B, SparseMatrix<float> &Cpar, float d_par, float Q_par, float R, float QN,
...
controllerB::controllerB(MatrixXf &A, MatrixXf &B, SparseMatrix<float> &Cpar, float d_par, float Q_par, float R, float QN, int Npred,
...
void controllerB::initialize(MatrixXf &A, MatrixXf &B, MatrixXf &qlin, float R, float QN) {
    /* Initialize Controller */
    VectorXf x0(n);                     // Dummy Variable
    ...
    // Hessian matrix and linear part of objective function
    ...
    // Solve first QP.
    qpOASES::real_t U[nV];
    Qp.init(Hsd, g_qp, Ulb_qp, Uub_qp, nWSR);

    Qp.getPrimalSolution(U);
}


float controllerB::getControlInput(VectorXf &x0, float yrr)
{
    ...
    Qp.hotstart(g_qp, Ulb_qp, Uub_qp, nWSR);
